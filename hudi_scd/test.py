...
yellow_tripdata_schema = StructType(
    [StructField("vendorid", IntegerType(), True), StructField("tpep_pickup_datetime", TimestampType(), True),
     StructField("tpep_dropoff_datetime", TimestampType(), True), StructField("passenger_count", IntegerType(), True),
     StructField("trip_distance", DoubleType(), True), StructField("ratecodeid", IntegerType(), True),
     StructField("store_and_fwd_flag", StringType(), True), StructField("pulocationid", IntegerType(), True),
     StructField("dolocationid", IntegerType(), True), StructField("payment_type", IntegerType(), True),
     ...
     StructField("total_amount", DoubleType(), True), StructField("congestion_surcharge", DoubleType(), True),
     StructField("pk_col", LongType(), True)])

inputDf = spark.read.schema(yellow_tripdata_schema).option("header", "true").csv(
    "s3://nyc-tlc/trip data/yellow_tripdata_{2018,2019,2020}*.csv").withColumn("pk_col",
...
commonConfig = {'className': 'org.apache.hudi', 'hoodie.datasource.hive_sync.use_jdbc': 'false',
                ...
                'hoodie.datasource.write.recordkey.field': 'pk_col', 'hoodie.table.name': 'ny_yellow_trip_data',
                'hoodie.consistency.check.enabled': 'true', 'hoodie.datasource.hive_sync.database': 'default',
                'hoodie.datasource.hive_sync.table': 'ny_yellow_trip_data',
                'hoodie.datasource.hive_sync.enable': 'true',
                'path': 's3://' + args['curated_bucket'] + '/default/ny_yellow_trip_data'}
